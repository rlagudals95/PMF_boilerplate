...
async function main() {
  ...
  const summaryLines = [
    checkOnly ? "Checked AI context adapters:" : "Synced AI context adapters:",
  ...
  if (checkOnly && driftIssues.length > 0) {
    process.stdout.write(
      [
        ...summaryLines,
        "",
        "Drift:",
        ...driftIssues.map((item) => `- ${item}`),
      ].join("\n"),
    );
    process.exitCode = 1;
    return;
  ...
  if (checkOnly) {
    process.stdout.write(
      [...summaryLines, "", "- No adapter drift detected"].join("\n"),
    );
    return;
  }
  ...
  try {
    ...
    if (checkOnly) {
      staleEntries.forEach((entry) =>
        recordDrift(
          `stale generated directory: ${relativePath(path.join(skillsDir, entry.name))}`,
        ),
      );
      return;
    }
...
async function removeStaleFiles(directoryPath, shouldRemove) {
  try {
    ...
    const staleEntries = entries.filter(shouldRemove);

    if (checkOnly) {
      staleEntries.forEach((entry) =>
        recordDrift(
          `stale generated file: ${relativePath(path.join(directoryPath, entry))}`,
        ),
      );
      return;
    }
    ...
    await Promise.all(
      staleEntries.map((entry) =>
        rm(path.join(directoryPath, entry), { force: true }),
      ),
    );
  } catch (error) {
    if (!isMissingPathError(error)) {
...
async function ensureGeneratedDir(directoryPath) {
  if (checkOnly) {
    return;
  }
...
async function writeGeneratedFile(filePath, contents) {
  if (checkOnly) {
    await assertGeneratedFileMatches(filePath, contents);
    return;
  }

  await ensureGeneratedDir(path.dirname(filePath));
  await writeFile(filePath, contents);
}

async function assertGeneratedFileMatches(filePath, expectedContents) {
  try {
    const currentContents = await readFile(filePath, "utf8");

    if (currentContents !== expectedContents) {
      recordDrift(`out-of-date file: ${relativePath(filePath)}`);
    }
  } catch (error) {
    if (isMissingPathError(error)) {
      recordDrift(`missing generated file: ${relativePath(filePath)}`);
      return;
    }
...
function recordDrift(message) {
  if (!driftIssues.includes(message)) {
    driftIssues.push(message);
  }
}
...
async function writeAdapterReadmes(skills, agents) {
  ...
  const codexReadme = [
    ...
    "- Use `pnpm repo:check` before handoff when canonical docs, work items, or adapters changed.",
  ...
  await writeGeneratedFile(
    path.join(rootDir, ".claude", "README.md"),
    claudeReadme,
  );
  await writeGeneratedFile(
    path.join(rootDir, ".gemini", "README.md"),
    geminiReadme,
  );
  await writeGeneratedFile(
    path.join(rootDir, ".codex", "README.md"),
    codexReadme,
  );
  await writeGeneratedFile(
    path.join(rootDir, ".cursor", "README.md"),
    cursorReadme,
  );
}
...
async function writeClaudeSkill(skill, skillBody) {
  ...
  await writeGeneratedFile(path.join(skillDir, "SKILL.md"), skillBody);
}

async function writeClaudeAgent(agent) {
  const agentDir = path.join(rootDir, ".claude", "agents");

  await writeGeneratedFile(path.join(agentDir, `${agent.name}.md`), agent.body);
}
  ...
  const rules = [
    {
      ...
      body: [
        ...
        "",
        "- Read `AGENTS.md`, `ai/context/project.md`, `ai/context/ai-native.md`, `ai/context/engineering.md`, `ai/context/engineering-common.md`, `ai/context/spec-driven.md`, and `ai/context/doc-sync.md` before substantial work.",
        "- Normalize policy, business-goal, PRD, and raw-request inputs through `ai/context/ai-native.md` before choosing an MVP slice or work item path.",
        ...
        "- Run `pnpm repo:check` when canonical docs, work item contracts, or generated adapters changed.",
    ...
    },
    {
      ...
      body: [
        ...
        "- Update `ai/context/ai-native.md` first when repo promises, quality bar, or Docs-as-Code rules change.",
        "- Load `ai/context/doc-sync.md` and `docs/agent-context.md` when changing docs, workflows, or adapters.",
        "- Keep metadata (`owner`, `doc_type`, `source_of_truth`, `freshness`, `verification`) accurate on core docs and task-local artifacts.",
        "- Update canonical docs first, then regenerate adapter outputs with `pnpm ai:sync`.",
        "- Run `pnpm repo:check` after changing canonical docs, work item templates, or adapter-driving files.",
