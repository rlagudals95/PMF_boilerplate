...
2. 기본값은 `po supervisor -> bounded specialists -> evaluator gate`이며, `agent-team`은 exception path로만 엽니다.
3. `team-plan.md`에 lead, teammates, shared context pack, task graph, file ownership를 먼저 적습니다.
4. lead는 goal packet과 success metric을 기준으로 task를 작게 나눕니다.
5. 각 teammate는 결과만 넘기지 말고 다음 역할이 바로 이어받을 수 있는 handoff packet을 남깁니다.
6. 병렬 작업이면 같은 파일을 동시에 편집하지 않도록 owner를 나눕니다.
7. 구현보다 먼저 research/review task를 태워서 방향을 좁힙니다.
8. 마지막에는 `evaluator-role` 또는 동등한 reviewer가 `quality-scorecard.md`를 닫고, lead가 최종 synthesis를 합니다.

## Execution Modes

- `single-agent sequential`
  - 기본값. 한 에이전트가 `PO -> PM -> PD -> FE -> BE -> evaluator` 순서로 역할을 시뮬레이션합니다.
- `subagent fan-out`
...
- user-facing conversation과 synthesis는 lead/PO가 기본 소유자입니다.
