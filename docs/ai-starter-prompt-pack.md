...
- 기존 broad `gated work` 개념은 유지하되, hybrid harness contract에서는 `light | soft-gated | hard-gated`로 더 정확히 분류한다.
...
| `gated work` | legacy umbrella for user-facing flow, goal-critical change, policy/business goal -> MVP shaping, validation/persistence/analytics contract change, 여러 파일에 걸친 기능 작업; classify further as `soft-gated` or `hard-gated` | `goal packet -> brief -> role spec -> quality gate`를 먼저 열기 |
...
- release surface는 `none | user-facing | ops-facing | cross-repo`로 함께 적되, enforcement는 upcoming harness gate 작업에서 붙습니다.
...
7. 이 요청의 work class가 `light | soft-gated | hard-gated`인지 먼저 보고, release surface도 함께 선언한 뒤, 동시에 `product-config-friendly`인지 또는 `deep code`가 필요한지도 판단한 뒤 가장 안전한 surface부터 시작한다.
