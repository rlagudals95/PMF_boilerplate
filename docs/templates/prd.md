---
title: "New PRD"
status: draft
owner: ""
source_url: ""
created_at: ""
updated_at: ""
---

# Product Requirements Document

## Problem

-

## Goal

-

## Non-Goals

-

## Target User

-

## Core Use Cases

### Primary

- situation:
- user_action:
- expected_outcome:

### Secondary

- situation:
- user_action:
- expected_outcome:

## Jobs To Be Done

-

## Success Metric

-

## Scope

### In Scope

-

### Out Of Scope

-

## Feature Candidates

### example-feature

- summary:
- user_value:
- primary_module:
- routes:
- ui_surface: yes
- admin_surface: no
- backend_changes: yes
- auth_required: no
- payment_required: no
- external_provider_impact: none
- analytics_required: yes
- recommended: yes

## Testability Notes

- 어떤 acceptance criteria가 먼저 automated test로 고정되어야 하는가
- light work 예외인지, 아니면 test-first가 필요한 핵심 로직 변경인지
- feature work item으로 내려갈 때 먼저 나눌 behavior slice

## Acceptance Criteria

- [ ] 첫 번째 public behavior 기준
- [ ] 두 번째 public behavior 기준

Acceptance criteria 작성 규칙:

- 구현 디테일이 아니라 public behavior 기준으로 적습니다.
- 구현자가 추가 해석 없이 테스트 가능한 문장으로 적습니다.
- 중요한 작업이면 어떤 기준을 먼저 failing test로 고정할지 PRD 단계에서 힌트를 남깁니다.

## Analytics Impact

-

## Data Impact

-

## Dependencies

-

## Open Questions

-

## Document History

| Date | Type | Summary | Author |
| --- | --- | --- | --- |
| YYYY-MM-DD | created | Initial PRD created. | owner |

히스토리 규칙:

- PRD 생성 시 `created` 행을 남깁니다.
- 의미 있는 수정이 있을 때마다 새 행을 추가합니다.
- 기존 행을 덮어쓰지 않고 append-only로 유지합니다.
- 최신 변경이 항상 최하단에 오게 합니다.
